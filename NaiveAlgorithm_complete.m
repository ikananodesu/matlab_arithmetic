clc,clear,close all;
%[出发点 V1 V2 V3 V4 V5 接收]
%网络输入需求
Need=4;
%定义流量图
Original_G=[0,5,2,0,0,0;
            0,0,2,2,4,0;
            0,0,0,0,2,0;
            0,0,0,0,0,3;
            0,0,0,0,0,3;
...
%迭代器
while 1
[Map,flag]=Graph_iteration_F(Begin_P,L,Residual_G);
    if flag~=0
        break;
    end
Residual_G=Map;
...
%输出最大流量分配图
Answer_G=Original_G-Residual_G;
...
%算法实现函数
%流通图迭代函数
function [Map,flag]=Graph_iteration_F(Begin_P,L,Map)
...
for i=1:length(path(1,:))-1
    Map(path(1,i),path(1,i+1))=Map(path(1,i),path(1,i+1))-min(path(2,1:end-1));
end
end
